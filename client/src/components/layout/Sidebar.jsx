...
function NavRow({ item, badges, onNavigate }) {
  ...
  return (
    ...
    >
      {({ isActive }) => (
        <>
          <Icon className={`h-5 w-5 shrink-0 ${isActive ? 'text-brand-700' : 'text-slate-400'}`} aria-hidden="true" />
          <span className="truncate">{label}</span>
          <CountBadge value={badge} />
        </>
      )}
...
export function Sidebar({ open = false, onClose, inboxUnread = 0 }) {
  ...
  return (
    <>
      ...
      <aside
        className={`fixed inset-y-0 left-0 z-50 flex w-64 flex-col border-r border-line bg-white transition-transform lg:static lg:z-auto lg:translate-x-0 ${open ? 'translate-x-0' : '-translate-x-full'}`}
      >
        <div className="flex items-center gap-2.5 px-5 py-5">
          <span className="grid h-9 w-9 place-items-center rounded-xl bg-brand-600 text-base font-bold text-white">K</span>
        ...
        <nav className="flex-1 space-y-6 overflow-y-auto px-3 pb-4">
          {config.sections.map((section) => (
            <div key={section.label} className="space-y-1.5">
              <p className="px-3 font-schibsted text-xs font-semibold uppercase tracking-wider text-muted">{section.label}</p>
              {section.items.map((item) => (
                <NavRow key={item.label} item={item} badges={badges} onNavigate={onClose} />
              ))}
            </div>
          ))}
        </nav>
